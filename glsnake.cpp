...
glSnake::~glSnake() { tim -> stop();delete tim;}
...
void glSnake::timerEvent(QTimerEvent *e)
{
  Q_UNUSED(e)
    second++;
  if(glSnake::second%15==0){locateFruit();}
  if(glSnake::second==1000)glSnake::second=0;
  updatetime();
}
...
void glSnake::_processing() {
  ...
  case eState::PLAY: {
        ...
        if(y3 ==Fy2 && x3 == Fx2 && y4==Fy3 && x4 == Fx3){
            snakea.push_back(bodysnake{0,0,0,0,0,0,0,0});
            locateFruit();
            glSnake::score++;
            _draw_score();
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ...
        for(QVector<bodysnake>::iterator p=snakea.begin()+1;p!=snakea.end();++p)
        {
//            x1=40;y1=30;
//            x2=50;y2=30;
//            x3=50;y3=40;
//            x4=40;y4=40;
            if(x1==p->px&&y1==p->py&&x2==p->px1&&y2==p->py1&&x3==p->px2&&y3==p->py2&&x4==p->px3&&y4==p->py3)
            {
                mState=eState::GAME_OVER;
                initgamesnake();
                break;
            }
        }
...
void glSnake::pausegame()
{
    ...
    auto dy = 55.f;
    qglColor(Qt::lightGray);
    renderText(x, y, "Pause", font_selected);
}

void glSnake::gameover()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    auto app_w = glSnake::width() / 2.f;
    auto app_h = glSnake::height() / 3.f;
    ...
    auto dy = 55.f;
    qglColor(Qt::lightGray);
    renderText(x, y, "Game over", font_selected);
    //paintGL();
